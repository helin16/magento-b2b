		...
		foreach($orders as $index => $order)
		{
			...
			try
			{
				...
				$customer = Customer::create(
						($order->billing_address && $order->billing_address->company && trim($order->billing_address->company) !== '') ? trim($order->billing_address->company) : (isset($order->customer_firstname) ? trim($order->customer_firstname) . ' ' . trim($order->customer_lastname) : ''), 
						'', 
