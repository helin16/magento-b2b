...
function getProductIds($argv)
{
	...
	else $productIds = Dao::getResultsNative('select distinct p.id from product p', array(), PDO::FETCH_ASSOC);
	
	return $productIds;
}

function downloadFile($argv,$cacheFile)
{
	$productIds = getProductIds($argv);
	file_put_contents($cacheFile, '');
	foreach($productIds as $productId)
	{
		$pro = Product::get($productId['id']);
		$proArray = $pro->getJson();
		if(count($proArray) > 0)
...
try {
	...
	$cacheFile = '/tmp/systemProduct.json';
	
	downloadFile($argv,$cacheFile);
	//process file
