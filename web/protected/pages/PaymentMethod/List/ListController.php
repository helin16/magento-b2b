...
class ListController extends CRUDPageAbstract
{
	...
	public function getItems($sender, $param)
	{
		...
		try
		{
			...
			$where = array(1);
			$params = array();
			if(isset($serachCriteria['py_method.name']) && ($name = trim($serachCriteria['py_method.name'])) !== '')
			{
				$where[] = 'py_method.name like ?';
				$params[] = '%' . $name . '%';
			}
			$stats = array();
			$objects = $class::getAllByCriteria(implode(' AND ', $where), $params, false, $pageNo, $pageSize, array('py_method.name' => 'asc'), $stats);
