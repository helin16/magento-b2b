...
PageJs.prototype = Object.extend(new BPCPageJs(), {
	...
	,_getProductRow: function(orderItem, isTitleRow) {
		...
		tmp.stockInRMAEl = new Element('div', {'class': 'form-group'} )
			...
			.observe('keydown', function(event){
				tmp.txtBox = $(this);
				tmp.me.keydown(event, function() {
					tmp.txtBox.up('.product-head-row').down('[save-item="stockInParts"]').select();
				});
			})
			.observe('click', function(event){
				$(this).down('input').select();
	...
	,_getProductsTable: function () {
		...
		tmp.productListDiv = new Element('div', {'class': 'list-group', 'id': tmp.me._htmlIds.productsTable})
			.insert({'bottom': tmp.newDiv = tmp.me._getProductRow({'product': {'sku': 'SKU', 'name': 'Product Name', 'stockInParts': 'Parts', 'stockOnHand': 'Hand', 'stockOnOrder': 'Order', 'stockInRMA': 'RMA'} }, true).addClassName('item-title-row') });
