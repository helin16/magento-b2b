	...
	,_displayPriceMatchResult: function(prices, product) {
		...
		$H(prices["companyPrices"]).each(function(price){
			...
			tmp.tbody.insert({'bottom': new Element('tr')
				.insert({'bottom': new Element('td', {'colspan': 3}).update(price.key).addClassName(price.key===product.priceMatchRule.priceMatchCompany.companyName ? 'success' : '') })
		...
		tmp.newDiv = new Element('div', {'class': ''})
			...
			.insert({'bottom': new Element('div', {'class': 'col-xs-6'})
				.insert({'bottom': new Element('div', {'class': 'form-group form-group-sm input-group'})
					...
					.insert({'bottom': new Element('input', {'type': 'text', 'class': 'form-control input-sm', 'match_rule': 'price_from', 'value': tmp.product ? tmp.product.priceMatchRule.price_from : ''})
			...
			.insert({'bottom': new Element('div', {'class': 'col-xs-6'})
				.insert({'bottom': new Element('div', {'class': 'form-group form-group-sm input-group'})
					...
					.insert({'bottom': new Element('input', {'type': 'text', 'class': 'form-control input-sm', 'match_rule': 'price_to', 'value': tmp.product ? tmp.product.priceMatchRule.price_to : ''})
						.observe('keyup', function(e){
			...
			.insert({'bottom': new Element('div', {'class': 'col-xs-9'})
				.insert({'bottom': new Element('div', {'class': 'form-group form-group-sm input-group'})
					...
					.insert({'bottom': new Element('input', {'type': 'text', 'class': 'form-control input-sm', 'match_rule': 'offset', 'value': tmp.product ? tmp.product.priceMatchRule.offset : ''}) })
				})
			})
	...
	,_getResultRow: function(row, isTitle) {
		...
		tmp.row = new Element('tr', {'class': 'visible-xs visible-md visible-lg visible-sm ' + (tmp.isTitle === true ? '' : 'product_item ' + (row.stockOnHand <= row.stockMinLevel ? 'danger': (row.stockOnHand <= row.stockReorderLevel ? 'warning' : '' ))), 'product_id' : row.id})
			...
			.insert({'bottom': new Element(tmp.tag, {'class': 'hidden-xs hide-when-info hidden-sm row'}).addClassName('col-xs-2').setStyle(tmp.me._showRightPanel ? 'display: none' : '')
				...
				.insert({'bottom': new Element('div', {'class': 'col-sm-3'}).update(tmp.isTitle === true ? 'Match' : new Element('span').update(row.priceMatchRule.priceMatchCompany ? row.priceMatchRule.priceMatchCompany.companyName : '').setStyle('width: 100%') ) })
