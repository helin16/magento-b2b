		...
		tmp.newDiv = new Element('table', {'class': 'table table-striped table-hover price-match-listing'})
			...
			.insert({'bottom': new Element('tbody')
				.insert({'bottom': new Element('tr')
					...
					.insert({'bottom': new Element('td', {'class': 'price_diff'}).update(new Element('span', {'class': '' + (prices.priceDiff * 1) > 0 ? 'label label-danger' : ''}).update(tmp.me.getCurrency(prices.priceDiff)) ) })
