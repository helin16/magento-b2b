...
PageJs.prototype = Object.extend(new CRUDPageJs(), {
	_getTitleRowData: function() {
		return {'type': "Type", 'from': 'From', 'to': 'To', 'subject': 'Subject', 'body': 'Body', 'status': 'Status', 'created': 'Created', 'updated': 'Updated'};
	}
	...
	,_getResultRow: function(row, isTitle) {
		...
		tmp.row = new Element('tr', {'class': (tmp.isTitle === true ? '' : 'btn-hide-row')}).store('data', row)
			...
			.insert({'bottom': new Element(tmp.tag, {'class': 'name col-xs-1'}).update(row.from) })
			.insert({'bottom': new Element(tmp.tag, {'class': 'name col-xs-1'}).update(row.to) })
			.insert({'bottom': new Element(tmp.tag, {'class': 'name col-xs-1'}).update(row.subject) })
			.insert({'bottom': new Element(tmp.tag, {'class': 'name col-xs-3'}).update(row.body) })
			.insert({'bottom': new Element(tmp.tag, {'class': 'name col-xs-1'}).update(row.status) })
