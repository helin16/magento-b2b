...
PageJs.prototype = Object.extend(new CRUDPageJs(), {
	...
	,_bindSearchKey: function() {
		...
		tmp.selectEl = new Element('input', {'class': 'select2 form-control', 'data-placeholder': 'search for a Customer', 'search_field': 'cust.id'}).insert({'bottom': new Element('option').update('')});
	...
	,_getResultRow: function(row, isTitle) {
		...
		tmp.row = new Element('tr', {'class': (tmp.isTitle === true ? 'item_top_row' : 'btn-hide-row item_row') + (row.active == 0 ? ' danger' : ''), 'item_id': (tmp.isTitle === true ? '' : row.id)}).store('data', row)
			.insert({'bottom': new Element(tmp.tag, {'class': 'col-xs-1'}).update(row.creditNoteNo)})
