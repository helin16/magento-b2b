...
PageJs.prototype = Object.extend(new BPCPageJs(), {
	...
	,deleteUser: function(item, btn) {
		...
		tmp.me.postAjax(tmp.me.getCallbackId('deleteUser'), {"userId": item.id}, {
			'onLoading': function(sender, param) {
				if($(btn)) {
					$(btn).store('originVal', $(btn).innerHTML).update('Deleting ...').addClassName('disabled');
				}
			}
			...
			,'onComplete': function (sender, params) {
				if($(btn)) {
					$(btn).removeClassName('disabled').update($(btn).retrieve('originVal'));
				}
			}
		});
		return tmp.me;
	...
	,_getItemRow: function(item, isTitle) {
		...
		if(tmp.isTitle !== true) {
			...
			//btnsdiv
			tmp.btnDiv
				.insert({'bottom': new Element('span', {'class': 'button edit'})
					.update('Edit')
					.observe('click', function() {
						tmp.me._showEditPanel(item);
					})
				})
				.insert({'bottom': new Element('span', {'class': 'button delete'})
					.update('Delete')
					.observe('click', function() {
		...
		tmp.newDiv = new Element('div', {'class': 'row', 'useraccount_id': item.id}).store('data', item)
			.insert({'bottom': new Element('span', {'class': 'cell firstName'}).update(item.person.firstName) })
			.insert({'bottom': new Element('span', {'class': 'cell lastName'}).update(item.person.lastName) })
			.insert({'bottom': new Element('span', {'class': 'cell roles'}).update(tmp.roles) })
			.insert({'bottom': new Element('span', {'class': 'cell username'}).update(item.username) })
			.insert({'bottom': new Element('span', {'class': 'cell btns'}).update(tmp.btnDiv)});
		return tmp.newDiv;
	}
