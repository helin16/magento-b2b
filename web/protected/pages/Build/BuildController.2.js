	...
	,_bindDownloadBtn: function() {
		...
		jQuery('#'+tmp.me.downloadBtnId).click(function(){
			...
			jQuery('#'+pageJs.resultDivId+' input.select2').select2('data').each(function(item){
				tmp.obj = {};
				$H(item.data).each(function(item2){
					if(typeof item2.value === 'string') {
						tmp.obj[item2.key] = item2.value;
					}
				});
				tmp.obj['sku'] = item.data.sku;
				tmp.obj['price'] = (item.data.prices.length > 0 ? item.data.prices[0].price : 'N/A');
				tmp.data.push(tmp.obj);
			});
